...
const adminAvailable = process.env.ADMIN_ENDPOINTS_AVAILABLE === 'true';
...
describe.skipIf(!adminAvailable)(
  'Admin routes (require core ≥ v2.2.0 with /api/admin/* endpoints)',
  () => {
    ...
    it('deletes via HTMX and returns an empty 200', async () => {
      ...
      // Subsequent detail read should 404.
...
describe.skipIf(adminAvailable)('Admin routes — note', () => {
  it('skipped: pulled core image lacks /api/admin/* (pre-v2.2.0)', () => {
    expect(adminAvailable).toBe(false);
  });
});
